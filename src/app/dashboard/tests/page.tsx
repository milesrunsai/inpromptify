...
export default function MyTestsPage() {
  ...
  const listingBadge = (t: DbTest) => {
    ...
    const colors: Record<string, string> = {
      ...
      test: "bg-orange-50 text-orange-700 border-orange-100",
        ...
        {(["all", "active", "draft", "archived"] as FilterTab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-3 py-2 text-[13px] font-medium border-b-2 transition-all duration-200 ${
              tab === t ? "border-orange-500 text-orange-400" : "border-transparent text-gray-400 hover:text-gray-400"
            }`}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Link href={`/dashboard/tests/${test.id}`} className="text-[13px] font-medium text-white hover:text-orange-400 transition-colors truncate">
                            {test.title}
                          </Link>
                        </div>
                  ...
                  <td className={`${ds.tableCell} text-right`}>
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200">
                      <Link href={`/dashboard/tests/${test.id}`}
                        className="px-2 py-1 text-[11px] font-medium text-gray-500 hover:text-orange-400 rounded hover:bg-orange-500/10 transition-colors"
                        title="Edit">
                        Edit
                      </Link>
